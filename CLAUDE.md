...
- **Comptes et synchro** : Supabase, projet `zjzfwhtqrigfmqzfebzy` (offre gratuite), clé publique dans `js/config.js`.
  Scripts SQL à exécuter une fois, dans l'ordre, dans *SQL Editor* : `supabase-setup.sql` → `supabase-certif.sql` → `supabase-v2.sql` → `supabase-v3.sql` (capsules) → `supabase-v4.sql` (amis).
...
- `js/certify.js` : certification en direct (défi tiré par le serveur, film d'environ 1,3 s, mouvement, flux figé, détection d'écran, empreinte dHash, vérification que la carte choisie est bien la plus ressemblante de sa série).
...
- Navigation (sept. 2026) : `#/compte` = la **vitrine** (page du profil, via l'avatar en haut à droite) ; `#/vitrine` reste un alias. La connexion / synchro (`views/account.js`) est intégrée en haut des **Paramètres** (`embedded: true`) et a aussi sa page `#/connexion` (liens « Connecte-toi », réinitialisation du mot de passe). L'onglet du menu **Match** (`#/match`, `views/match.js`) : combats simplifiés contre l'ordinateur. Moteur `js/battle.js` (`App.battle` : `fighter(card)` depuis `getCard` TCGdex, types FR/EN normalisés, `damage` avec faiblesse/résistance et pièces pour « × » / « + », `aiMove`/`aiReplace` par niveau, `LEVELS` 1-5 : communes / évolutions / holos du Set de Base, Maître = meilleures holos + 1 énergie, Légende = ex de 151). Équipe de 3 (`profile.match = {team, beaten, wins, losses}`, synchronisé) ; Pokémon de prêt si besoin. Simulations : holos ≈ 55 % contre Champion, 0 % contre Légende ; ex ≈ 43 % contre Légende. Combats entre amis : plus tard. La pastille du compte pointe vers `#/compte` si connecté, `#/connexion` sinon.
- Amis : `supabase-v4.sql` (table `friendships` a→b pending/accepted, `are_friends`, `friend_request(pseudo)` qui accepte direct si demande croisée, `friend_respond`, `friend_remove`, `friend_list`, `friend_showcase(uid)` qui renvoie profil + cartes + certifs seulement si amis, règle de stockage « photos de mes amis »). `js/friends.js` (`App.friends`, invitation par lien `#/amis?ajout=Pseudo` gardée dans `kv.invite` jusqu'à la connexion), `js/views/friends.js` (`#/amis`), vitrine d'un ami = `views/showcase.js` avec `params.friend` (`#/ami/<uid>`, lecture seule, source de données `S`). `App.col.progress(game, set, items)` et `App.badges.unlocked(items, isCert)` acceptent les cartes d'un autre dresseur.
...
- Classeur, grille automatique : `recognizer.detectPage(img, FORMATS, fmt)` → `gridProfiles` (profils des longs bords droits, gradient signé moyenné le long du bord) + `fitAxis` (cartes régulières p/w/a, pénalité si une rangée de plus existe) + couplage forme de carte ; format choisi par la part des grands bords expliquée (`fit`). Si `fit ≥ 0,6` : grille posée et reconnaissance lancée après 3 s (« Ajuster d'abord » annule). Chaque pochette : `refineCell` (paire de bords par axe, axe faible déduit de 63×88), puis `locateCard(…, 0.8)`. Testé sur une vraie page (photo propre + versions petite, tournée 3°, en biais, 4×3 recomposée).
- Versions : `recognizer.detectVariants(blob, card.variants)` appelé dans `addScanned` → `item.variants` + `item.variantsAuto` ; `R.lastVariants` pour l'affichage. Base holo/normale : déduite si une seule existe ; reverse : `foilIn` sur la zone du texte (grain + variation de saturation ≥ 14 ; négatifs mesurés 5 à 11, pas encore de vraie reverse pour calibrer) ; 1ʳᵉ édition : `firstEditionStamp` (tache sombre ronde peu colorée sous l'illustration, gauche ou droite, seuil 0,3 ; sur la photo test à ~260 px de large le vrai logo ne fait que 0,12-0,14 et les faux jusqu'à 0,20 → réglé pour ne pas se tromper, à recalibrer avec une photo nette).
