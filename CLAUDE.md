...
- Navigation (sept. 2026) : `#/compte` = la **vitrine** (page du profil, via l'avatar en haut à droite) ; `#/vitrine` reste un alias. La connexion / synchro (`views/account.js`) est intégrée en haut des **Paramètres** (`embedded: true`) et a aussi sa page `#/connexion` (liens « Connecte-toi », réinitialisation du mot de passe). L'onglet du menu **Match** (`#/match`, `views/match.js`) est un écran d'attente : idée d'Arnaud = rendre les cartes jouables (decks, combats contre amis / ordinateur avec niveaux). La pastille du compte pointe vers `#/compte` si connecté, `#/connexion` sinon.
